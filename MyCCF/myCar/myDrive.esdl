...
class myDrive {
	...
	public void move(real in powerCtrl, real in brakeCtrl, s in mydt, g in myg, real in recupCtrl) {
		...
		momentum = EngineMomentum.getAt(power, v); // Main/move 8
		recup_momentum = (curve_brake_recuperation_table.getAt(v) * (recupCtrl / 100.0) * recupMult); // Main/move 9
		battery = myBattery_instance.get_battery((momentum + recup_momentum), ds); // Main/move 10
		BatteryState_instance.battery = battery; // Main/move 11
		BatteryState_instance.powerInput = powerCtrl; // Main/move 12
		BatteryState_instance.batteryStateStatemachineTrigger(); // Main/move 13
